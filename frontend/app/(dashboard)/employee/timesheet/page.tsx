...
import { DatePicker, Drawer, Tabs, Tag, Select, Spin, Radio, Modal } from "antd";
import dayjs from "dayjs";
import EnterTimesheetTab from "./EnterTimesheetTab";
...
interface TimesheetApiResponse {
  ...
  data: {
    entries: Array<{
      ...
      activity_product_code?: string;
      activity_product_name?: string;
      activity_is_billable?: boolean;
      [key: string]: any;
    }>;
    pagination: {
      total_count: number;
...
export default function EmployeeTimesheetPage() {
  ...
  // Map timesheet API response to TimeEntry format
  const mapApiEntryToTimeEntry = (apiEntry: any): TimeEntry => {
    ...
    const rawStatus = String(
      apiEntry.latest_approval_status || apiEntry.approval_status || ""
    ).toUpperCase();
  ...
  const fetchTimesheetEntries = useCallback(async () => {
    ...
    try {
      ...
      if (fromDate) {
        params.append('entry_date_from', fromDate.format('YYYY-MM-DD'));
      }
      if (toDate) {
        params.append('entry_date_to', toDate.format('YYYY-MM-DD'));
      }
      
      // If no date filters, fetch data based on filter mode
      if (!fromDate && !toDate) {
        ...
        params.append('entry_date_from', startDate.format('YYYY-MM-DD'));
        params.append('entry_date_to', endDate.format('YYYY-MM-DD'));
      }

      // Build parallel requests: timesheet + leave (map entry_date filters to leave date filters)
      const tsEndpoint = `get_timesheet_entries${params.toString() ? `?${params.toString()}` : ''}`;
      const leaveParams = new URLSearchParams();
      // Map date filters for leave: use from_date range aligned to timesheet entry_date filters
      const entryFrom = params.get('entry_date_from');
      const entryTo = params.get('entry_date_to');
      if (entryFrom) leaveParams.append('from_date_from', entryFrom);
      if (entryTo) leaveParams.append('to_date_to', entryTo);
      const lvEndpoint = `get_leave_applications${leaveParams.toString() ? `?${leaveParams.toString()}` : ''}`;

      const [tsResp, lvResp] = await Promise.all([
        apiRequest<TimesheetApiResponse>(tsEndpoint, 'GET'),
        apiRequest<LeaveApiResponse>(lvEndpoint, 'GET'),
      ]);
      ...
      // Deduplicate by entry id so the same entry doesn't appear twice
      // (e.g. once as Draft and once as Pending/Approved).
      const statusRank: Record<TimeEntry["status"], number> = {
        Draft: 1,
        Pending: 2,
        Approved: 3,
        Rejected: 3,
      ...
      [...tsEntries, ...lvEntries].forEach((entry) => {
        const key = entry.id;
        ...
        // Keep the entry with the "stronger" status
  ...
  }, [fromDate, toDate, weekOffset, filterMode, selectedMonth, selectedWeek]);
  ...
  const getColorClass = (status: string, type: string, isToday: boolean = false) => {
    // For today's timesheet entries, always use blue border
    if (isToday && type === "timesheet") return "border-l-4 border-blue-500 bg-blue-50/30";
    // Otherwise, use status-based borders
  ...
  const handleDayCardClick = (dayDate: dayjs.Dayjs) => {
    // Allow adding entries for any date within the rolling past 7 days (timesheet) or today/future (leave)
    const today = dayjs().startOf('day');
    const todayEnd = dayjs().endOf('day');
    const sevenDaysAgo = dayjs().subtract(7, 'days').startOf('day');
    const isPast7Days = !dayDate.isAfter(todayEnd, 'day') && !dayDate.isBefore(sevenDaysAgo, 'day');
    const isTodayOrFuture = dayDate.startOf('day') >= today;
    // Only allow if date is within past 7 days (for timesheet) or today/future (for both)
    if (!isPast7Days && !isTodayOrFuture) return;
    ...
    <div className="p-2 sm:p-4 lg:p-6">
      ...
      <div className="overflow-x-auto">
        ...
        {filterMode === "month" ? (
          ...
          <div className={`bg-white rounded-lg border border-gray-200 shadow-sm ${loading ? 'opacity-50' : ''}`}>
            ...
            <div className="grid grid-cols-7 gap-px bg-gray-200">
              {getCalendarDays.map((calDay) => {
                const allDayEntries = entriesByDate[calDay.dateStr] || [];
                // Apply type filter
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {dayEntries.slice(0, 2).map((entry) => {
                        // Check if entry is for today's date - use entryDate if available, otherwise use the calendar day's date
                        const entryDate = entry.entryDate ? dayjs(entry.entryDate) : calDay.date;
                        const today = dayjs().startOf('day');
                        const entryDateStart = entryDate.startOf('day');
                        const isEntryToday = entryDateStart.isSame(today, 'day');
                        const isTimesheetToday = entry.type === "timesheet" && isEntryToday;
                        
                        const getEntryBorderClass = (status: string, type: string) => {
                          // For today's timesheet entries, always use blue border (highest priority)
                          if (isTimesheetToday) return "border-l-4 border-blue-500";
                          // Otherwise, use status-based borders
                          if (status === "Draft") return "border-l-4 border-blue-500";
                          ...
                          if (type === "timesheet") return "border-l-4 border-blue-500";
                        ...
                        return (
                          <div
                            ...
                            className={`text-[9px] p-1 rounded cursor-pointer hover:opacity-80 transition-opacity truncate ${getEntryBorderClass(entry.status, entry.type)} ${
                    ...
                    {(() => {
                      ...
                      const todayEnd = dayjs().endOf('day');
                      const sevenDaysAgo = dayjs().subtract(7, 'days').startOf('day');
                      const isPast7Days = !calDay.date.isAfter(todayEnd, 'day') && !calDay.date.isBefore(sevenDaysAgo, 'day');
                      const isTodayOrFuture = calDay.date.startOf('day') >= today;
                      // Enable button for past 7 days (timesheet only) or today/future dates (both timesheet and leave)
                      const isEnabled = isPast7Days || isTodayOrFuture;
                      
                      return (
                        <button 
                          onClick={(e) => {
                            e.stopPropagation();
                            handleAddEntry(calDay.date);
                          }}
                          disabled={!isEnabled}
                          ...
                          title={
                            isTodayOrFuture 
                              ? `Add entry for ${calDay.dateStr} (Timesheet or Leave)` 
                              : isPast7Days 
                                ? `Add timesheet entry for ${calDay.dateStr} (Leave cannot be applied for past dates)` 
                                : 'Date is outside the allowed range (past 7 days for timesheet, today or future for leave)'
                          }
                        >
          ...
          {days.map((day) => {
            ...
            const isPast = day.date.isBefore(today.startOf('day'), 'day');
            ...
            <div key={day.key} className={`border rounded-lg min-w-[100px] max-w-full overflow-hidden ${isToday ? 'border-blue-500 ring-2 ring-blue-500' : 'border-gray-200'}`}>
              {/* Day Header */}
              <div 
                className={`p-2 sm:p-3 border-b ${isToday ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-gray-50'} ${day.isPast7Days ? 'cursor-pointer hover:bg-gray-100 transition-colors' : 'opacity-60'}`}
                onClick={() => day.isPast7Days && handleDayCardClick(day.date)}
                title={day.isPast7Days ? `Click to add entry for ${day.dateStr}` : 'Date is outside the allowed range (past 7 days)'}
              >
              ...
              <div className="p-2 sm:p-3 min-h-[200px] sm:min-h-[300px] space-y-2">
                {dayEntries.map((entry) => {
                  // Check if entry is for today's date - use entryDate if available, otherwise use the day's date
                  const entryDate = entry.entryDate ? dayjs(entry.entryDate) : day.date;
                  const today = dayjs().startOf('day');
                  const entryDateStart = entryDate.startOf('day');
                  const isEntryToday = entryDateStart.isSame(today, 'day');
                  ...
                  <div
                    ...
                    className={`group relative bg-white border border-gray-200 rounded-lg p-2.5 sm:p-3 cursor-pointer hover:shadow-lg hover:border-gray-300 transition-all duration-200 overflow-hidden ${getColorClass(entry.status, entry.type, isEntryToday)}`}
                  >
                ...
                {(() => {
                  ...
                  const todayEnd = dayjs().endOf('day');
                  const sevenDaysAgo = dayjs().subtract(7, 'days').startOf('day');
                  const isPast7Days = !day.date.isAfter(todayEnd, 'day') && !day.date.isBefore(sevenDaysAgo, 'day');
                  const isTodayOrFuture = day.date.startOf('day') >= today;
                  // Enable button for past 7 days (timesheet only) or today/future dates (both timesheet and leave)
                  const isEnabled = isPast7Days || isTodayOrFuture;
                  ...
                  return (
                    <button 
                      ...
                      title={
                        isTodayOrFuture 
                          ? `Add entry for ${day.dateStr} (Timesheet or Leave)` 
                          : isPast7Days 
                            ? `Add timesheet entry for ${day.dateStr} (Leave cannot be applied for past dates)` 
                            : 'Date is outside the allowed range (past 7 days for timesheet, today or future for leave)'
                      }
                    >
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
      ...
      >
        {selectedEntry ? (
          ...
          (() => {
            // If date is in the future (not today), show only "Apply Leave" tab
            const today = dayjs().startOf('day');
            const isFutureDate = selectedDate && selectedDate.startOf('day') > today;
            
            if (isFutureDate) {
              // Show only Apply Leave tab for future dates
              return (
                <ApplyLeaveTab 
                  onClose={handleCloseDrawer} 
                  onSuccess={handleLeaveSubmitted} 
            ...
            // Show both tabs for today and past dates
            return (
              <Tabs
                activeKey={activeTab}
                ...
                items={[
                  ...
                  },
                  {
                    key: "leave",
                    label: "Apply Leave",
                    disabled: selectedDate ? selectedDate.startOf('day') < dayjs().startOf('day') : false,
                    children: <ApplyLeaveTab onClose={handleCloseDrawer} onSuccess={handleLeaveSubmitted} initialDate={selectedDate} />,
                  },
                ]}
              />
            );
