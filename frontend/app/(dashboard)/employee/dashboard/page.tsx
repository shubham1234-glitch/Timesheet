...
export default function EmployeeDashboardPage() {
  ...
  const [selectedMember, setSelectedMember] = useState<string>(user?.userCode || "");
  ...
  useEffect(() => {
    if (user?.userCode && !selectedMember) {
      setSelectedMember(user.userCode);
    }
  ...
  const timesheetStatusChart = useMemo(() => {
    ...
    // Build arrays with only non-zero values for cleaner chart display
    const allLabels = [
    ...
    const allColors = [
      "#10b981", "#f59e0b", "#ef4444", // Timesheet: Green, Orange, Red
      "#3b82f6", "#fbbf24", "#f43f5e"  // Leave: Blue, Amber, Rose (distinct colors)
    ];
    ...
    // If all values are zero, show a message or return null
  ...
  const taskStatusChart = useMemo(() => {
    ...
    console.log('Task Status Chart Data:', { completedTasks, tasksInProgress, tasksToDo, tasksBlocked });
    ...
    return {
      ...
      datasets: [{
        data: [
          completedTasks > 0 ? completedTasks : 0.01, 
          tasksInProgress > 0 ? tasksInProgress : 0.01, 
          tasksToDo > 0 ? tasksToDo : 0.01,
          tasksBlocked > 0 ? tasksBlocked : 0.01
        ],
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 mb-3">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-[10px] text-gray-600">Track your productivity and work progress</p>
          </div>
        </div>
      </div>
      ...
      {!selectedMember ? (
        <div className="flex flex-col items-center justify-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="text-center max-w-md px-4">
            ...
            <h3 className="text-base font-semibold text-gray-900 mb-1.5">Loading Dashboard</h3>
