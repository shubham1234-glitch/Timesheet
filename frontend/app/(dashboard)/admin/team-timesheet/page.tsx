...
import { getTeamMemberOptions } from "@/app/lib/masterData";
...
export default function TeamTimesheetPage() {
  ...
  // Get team members from master data (filtered by logged-in admin's team)
  const teamMemberOptions = useMemo(() => {
    return getTeamMemberOptions();
  }, []);
  ...
  const fetchTeamMemberTimesheet = useCallback(async () => {
    ...
    try {
      ...
      // Note: SUBMITTED entries should be shown as Pending (only SUBMITTED counts as pending, not DRAFT)
      const statusMap: Record<string, "Draft" | "Pending" | "Approved" | "Rejected"> = {
        DRAFT: "Draft",
        PENDING: "Pending",
        SUBMITTED: "Pending", // SUBMITTED entries are shown as Pending
        APPROVED: "Approved",
        REJECTED: "Rejected",
      };
      ...
      const mappedLeave: TimesheetEntry[] = leaveEntries
        ...
        .flatMap((e) => {
          ...
          for (let i = 0; i <= diff; i++) {
            ...
            const rawStatus = String(e.approval_status || '').toUpperCase();
            const mappedStatus = statusMap[rawStatus] || "Pending";
            // Debug: Log leave status mapping
            if (rawStatus === 'SUBMITTED' || rawStatus === 'APPROVED') {
              console.log(`🔍 Leave Status Debug: ID=${e.leave_application_id}, Raw=${rawStatus}, Mapped=${mappedStatus}, Type=${e.leave_type_name}, Date=${e.from_date}`);
            }
            perDay.push({
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-200 bg-gray-50 rounded-t-lg">
              <h3 className="text-lg font-semibold text-gray-900">Reject Timesheet Entry</h3>
              <button
                onClick={() => {
                  if (!submittingAction) {
                    setRejectModalVisible(false);
            ...
            <div className="p-5">
              ...
              <textarea
                ...
                placeholder="Please provide a reason for rejecting this timesheet entry..."
                rows={5}
              ...
              {!rejectionReason.trim() && (
                <p className="text-xs text-gray-500 mt-2">This field is required to reject the timesheet entry.</p>
              )}
            </div>
