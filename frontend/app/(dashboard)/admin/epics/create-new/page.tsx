...
export default function CreateNewEpicPage() {
  const router = useRouter();
  const pathname = usePathname();
  const roleBase = getRoleBase(pathname || '');

  const handleCreated = (epicId?: string | number) => {
    if (epicId) {
      router.push(`${roleBase}/epics?expandedEpic=${epicId}`);
    } else {
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
        <div className="flex items-center gap-3">
          <Button
            ...
            className="p-0"
          />
          <h1 className="text-xl font-bold text-gray-900">Create New Epic</h1>
        </div>
      </div>
